...
public class PSystemRule
{
    ...
    public PSystemRule(string reactiveMultiset, List<(float Probability, string Product)> possibleProducts, List<char> membraneLabels)
    {
        ...
        float totalProbability = possibleProducts.Sum(p => p.Probability);

        // Normalizar si la suma no es cero
        PossibleProducts = totalProbability > 0 
        ? possibleProducts.Select(p => (p.Probability / totalProbability, p.Product)).ToList() 
        : possibleProducts;
    }
    ...
    public string SelectRandomProduct()
    {
        ...
        if (Math.Abs(totalProbability - 1f) > 0.01f)
        {
